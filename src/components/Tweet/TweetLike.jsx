import { toggleCommentLike, toggleTweetLike } from "@/app/Slices/likeSlice";
...
const TweetLike = ({
  ...
  const toggleLike = (isLikeAction) => {
    ...
    dispatch(toggleTweetLike({ tweetId, toggleLike: isLikeAction })).then(
      (res) => {
        if (res.payload) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        ...
        aria-label={`Like tweet (${like.totalLikes} likes)`}
      ...
      <button
        ...
        aria-label={`Dislike tweet (${disLike.totalDisLikes} dislikes)`}
